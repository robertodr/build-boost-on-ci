...
if "%CMAKE_GENERATOR%"=="Visual Studio 15 2017 Win64" (
  ...
  if exist "C:\Deps\boost_1_67_0\include\boost-1_67\" (
    ...
    b2 -q install link=static,shared threading=multi variant=release toolset=msvc address-model=64 architecture=x86 --with-filesystem --with-test --with-system --with-python --prefix="C:\Deps\boost_1_67_0" > NUL
    rem Clean up
